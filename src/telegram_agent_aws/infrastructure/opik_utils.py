...
def configure() -> None:
    if settings.COMET_API_KEY and settings.COMET_PROJECT:
        ...
        try:
            ...
            logger.warning("Couldn't configure Opik. There is probably a problem with the COMET_API_KEY or COMET_PROJECT environment variables or with the Opik server.")
    else:
